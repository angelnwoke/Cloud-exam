...
sudo mysql -uroot -e "CREATE USER 'angel'@'localhost' IDENTIFIED BY 'nwoke';"
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON Laravel.* TO 'angel'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '26 s/=root/=angel/' /var/www/laravel/.env
sudo sed -i '27 s/=/=nwoke/' /var/www/laravel/.env
